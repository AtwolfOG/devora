...
export default function GoogleCallbackPage() {
    ...
    useEffect(() => {
        if (code && state) {
            api.get(`/auth/callback/google?code=${code}&state=${state}`).then((res) => {
                if (res.status !== 200) {
                    throw new Error("Failed to login with google")
                }
                customToast.success("Logged in with google successfully")
                router.push("/user/dashboard")
            }).catch((err) => {
                if (axios.isAxiosError(err)) {
                    customToast.error(err.response?.data?.error || "Failed to login with google")
                } else {
    ...
    return (
        <main className="flex flex-col items-center justify-center h-screen w-full p-4 text-center">
            <h5 className="text-xl mb-2">Signing In with Google</h5>
